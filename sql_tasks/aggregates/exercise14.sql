...
select distinct surname, firstname, bk.memid, starttime from cd.bookings bk
join cd.members me
on me.memid = bk.memid 
where starttime = (
  	select min(starttime) from cd.bookings b1
  where b1.memid = bk.memid and starttime > '2012-09-01'
  )
order by bk.memid;
