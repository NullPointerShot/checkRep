...
with booking_hours as (select bk.memid, firstname, surname, round(sum(slots)*0.5, -1) as hours from cd.bookings bk
join cd.members me
on me.memid = bk.memid
group by bk.memid, surname, firstname),
ranked_members as (select memid, firstname, surname, hours, rank() over(order by hours desc) as ranked
				  from booking_hours)
select firstname, surname, hours, ranked from ranked_members
order by ranked, surname, firstname
