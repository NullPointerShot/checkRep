...
with inc as (SELECT 
    fa.name, 
    fa.initialoutlay, 
    fa.monthlymaintenance, 
    EXTRACT(YEAR FROM bk.starttime) AS year,
    EXTRACT(MONTH FROM bk.starttime) AS month,
    SUM(
        CASE
            WHEN bk.memid = 0 THEN fa.guestcost * bk.slots
            ELSE fa.membercost * bk.slots
        END
    ) AS income
FROM 
    cd.facilities fa
JOIN 
    cd.bookings bk ON bk.facid = fa.facid
GROUP BY 
    fa.name, 
    fa.initialoutlay, 
    fa.monthlymaintenance, 
    EXTRACT(YEAR FROM bk.starttime), 
    EXTRACT(MONTH FROM bk.starttime)
having EXTRACT(YEAR FROM bk.starttime) = '2012'
ORDER BY 
    fa.name, 
    year, 
    month),
avg_inc as (select name, avg(income) avgi, initialoutlay, monthlymaintenance from inc
group by name, initialoutlay, monthlymaintenance)
select name, initialoutlay/(avgi-monthlymaintenance) from avg_inc
