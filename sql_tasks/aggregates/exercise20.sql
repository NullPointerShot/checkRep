...
with revenue as (
  select fa.facid, fa.name, sum (case
								when bk.memid = 0 then fa.guestcost * bk.slots
								else fa.membercost * bk.slots
								end) as total
  from cd.bookings bk
  join cd.facilities fa
  on fa.facid = bk.facid 
  group by fa.facid, fa.name
  ),
revenueclassificators as (
  select re.facid, re.name, re.total, ntile(3) over (order by re.total) as revclass
  from revenue re)
select name, case 
				when revclass = 1 then 'low'
				when revclass = 2 then 'average'
				when revclass = 3 then 'high'
			end as revenue
from revenueclassificators
order by revclass desc, name 
