...
with recursive recommenders(recommender, member) as (
	select recommendedby, memid
		from cd.members 
	union all
	select me.recommendedby, re.member
		from recommenders re
		inner join cd.members me
			on me.memid = re.recommender
)
select re.member member, re.recommender, me.firstname, me.surname
	from recommenders re
	inner join cd.members me
		on re.recommender = me.memid
	where re.member = 22 or re.member = 12
order by re.member asc, re.recommender desc     
